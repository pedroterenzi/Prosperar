...
// Variáveis para Controle dos Gráficos do BI
let biChartReceitaIns = null;
let biChartDespesaIns = null;
let biChartVolumeIns = null;
...
async function carregarPainelAnalytics() {
    ...
    try {
        ...
        agendamentos.forEach(a => {
            ...
            if(statusAtual !== 'falta' && statusAtual !== 'cancelado') {
                const serv = DADOS_SERVICOS.find(s => s.nome.trim().toLowerCase() === (a.servico || '').trim().toLowerCase());
                const valS = serv ? parseFloat(serv.preco) : 0.00;
                const valP = parseFloat(a.valor_produtos || 0);
                const valG = parseFloat(a.valor_gorjeta || 0);
                const totalAtendimento = valS + valP + valG;
                
                faturamentoTotal += totalAtendimento;
                atendimentosFinalizados++;

                const dataBase = a.data;
                if(!dadosDiarios[dataBase]) dadosDiarios[dataBase] = { rec: 0, qtd: 0 };
                dadosDiarios[dataBase].rec += totalAtendimento;
        ...
        if(biChartReceitaIns) biChartReceitaIns.destroy();
        if(biChartDespesaIns) biChartDespesaIns.destroy();
        if(biChartVolumeIns) biChartVolumeIns.destroy();

        const commonOptions = {
            responsive: true, maintainAspectRatio: false,
            plugins: { legend: { display: false } },
            scales: { 
                x: { display: true, ticks: {font:{size: 9}, color: '#a1a1aa'}, grid: {display: false} }, 
                y: { display: false } 
            },
            layout: { padding: 0 }
        };

        const ctxRec = document.getElementById('biChartReceita');
        if(ctxRec) {
            biChartReceitaIns = new Chart(ctxRec, {
                type: 'line',
                data: { 
                    labels: labelsOrdenadas.map(d => d.substring(8,10)+'/'+d.substring(5,7)), 
                    datasets: [{ data: dadosRec, borderColor: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.1)', fill: true, tension: 0.4, borderWidth: 2, pointRadius: 2 }] 
                },
                options: commonOptions
            });
        }

        const ctxDesp = document.getElementById('biChartDespesa');
        if(ctxDesp) {
            biChartDespesaIns = new Chart(ctxDesp, {
                type: 'line',
                data: { 
                    labels: labelsDespOrdenadas.map(d => d.substring(8,10)+'/'+d.substring(5,7)), 
                    datasets: [{ data: dadosDesp, borderColor: '#ef4444', backgroundColor: 'rgba(239, 68, 68, 0.1)', fill: true, tension: 0.4, borderWidth: 2, pointRadius: 2 }] 
                },
                options: commonOptions
            });
        }

        const ctxVol = document.getElementById('biChartVolume');
        if(ctxVol) {
            biChartVolumeIns = new Chart(ctxVol, {
                type: 'bar',
                data: { 
                    labels: labelsOrdenadas.map(d => d.substring(8,10)+'/'+d.substring(5,7)), 
                    datasets: [{ data: dadosQtd, backgroundColor: '#3b82f6', borderRadius: 4 }] 
                },
                options: { ...commonOptions, scales: { x: { display: true, ticks: {font:{size: 9}, color: '#a1a1aa'}, grid: {display: false} }, y: { display: false } } }
            });
        }
